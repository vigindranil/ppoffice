...
const AddCasePage = () => {
  ...
  const handleSelectChange = async (name, value) => {
    if (name === "ipcAct" && useIpcAct) {
      const selectedIpc = ipcActList.find(
        (ipc) => ipc.bnsId.toString() === value
      );
      ...
      const selectedBns = bnsSectionList.find(
        (bns) => bns.bnsId.toString() === value
      );
  ...
  const handleFileChange = (e) => {
    ...
    const validFiles = files.filter((file) => {
      ...
      if (!["image/jpeg", "image/jpg", "application/pdf"].includes(file.type)) {
        openAlert(
          "error",
          `${file.name} is not a valid format (JPG, JPEG, PDF only).`
        );
        return false;
      }
      return true;
  ...
  const handleAddCase = async () => {
    if (!validateForm()) {
      openAlert("error", "Please fill in all required fields.");
      return;
    }
                  ...
                  <Popover open={openPS} onOpenChange={setOpenPS}>
                    <PopoverTrigger asChild>
                      ...
                      >
                        {formData.psId
                          ? allPSList.find(
                              (ps) => ps.id.toString() === formData.psId
                            )?.ps_name
                          ...
                          <CommandGroup>
                            {allPSList.map((ps) => (
                              <CommandItem
                                key={ps.id}
                                onSelect={() =>
                                  handleSelectChange("psId", ps.id.toString())
                                }
                              >
                                <Check
                                  className={cn(
                                    "mr-2 h-4 w-4",
                                    formData.psId === ps.id.toString()
                                      ? "opacity-100"
                                      : "opacity-0"
                                  )}
                                />
                ...
                {useIpcAct ? (
                                ...
                                {ipcActList.map((ipc) => (
                                  ...
                                  >
                                    <Check
                                      className={cn(
                                        "mr-2 h-4 w-4",
                                        formData.ipcAct === ipc.bnsId.toString()
                                          ? "opacity-100"
                                          : "opacity-0"
                                      )}
                                    />
                  ...
                  <>
                    <div className="flex-1 space-y-2">
                      ...
                      <Popover
                        open={openBnsSection}
                        onOpenChange={setOpenBnsSection}
                      >
                        <PopoverTrigger asChild>
                          <Button
                            variant="outline"
