...
export default function DashboardCards() {
  ...
  const fetchDistrictDetails = async () => {
    try {
      const response = await postRequest("count-by-district", {});
      
      if (response) {
        setPsData(response.data)
        setGradients(response.data.map(() => generateGradient()))
        setError(null)
      } else {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
      {psData.map((card, index) => (
        <Link key={index} href={`/master-ps-dashboard/${card.DistrictID}`} className="block">
        <motion.div
          key={index}
        ...
        >
          ...
          >
            ...
            <CardHeader className="relative z-20">
              <CardTitle className="text-lg font-bold tracking-tight">{card.DistrictName}</CardTitle>
