...
class UserLoginModel 
{
    ...
    public function GenerateOTP($MobileNo)
    {
        try 
        {  
            Log::channel('daily')->info("sp_generateOTP called");
            $sql = DB::select('call sp_generateOTP(?,@IsOTP,@ErrorCode)', [$MobileNo]);
    ...
    {
        try {
            $sql = DB::select('call sp_validateOTP(?,?)', [
                $MobileNo,
                $OTP
            ]);
            Log::channel('daily')->info("DB Response::" . json_encode($sql, true));
