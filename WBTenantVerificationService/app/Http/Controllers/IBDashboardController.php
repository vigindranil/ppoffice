...
use App\IBDashboardModel;
use Illuminate\Support\Facades\Log;
use App\Http\Controllers\Response;
use Exception;
class IBApplicationDetailsByStatusIdResp{
...
class IBDashboardController
{
    //
    public function __construct(IBDashboardModel $Obj)
    {
        $this->DashboardCountsMapper = $Obj;
    }
    public function GetIBApplicationDetailsByStatusID(Request $request) {
        try {
            Log::channel('daily')->info("******** GetIBApplicationDetailsByStatusID Called ***************");
            ...
            $Applications = $this->DashboardCountsMapper->GetIBApplicationDetailsByStatusID(
                $inputData['StatusID'],
                $inputData['IBUserID'],
                $inputData['FromDate'] ?? "",
                $inputData['ToDate'] ?? "",
            ...
            if (!empty($Applications)) {
                foreach ($Applications as $application) {
                    $ApplicationsObj = new IBApplicationDetailsByStatusIdResp();
                    ...
                    $ApplicationsObj->tenant_rented_address_police_station = $application->RentedAddressPoliceStationName ?? "";
            ...
            Log::channel('daily')->info("******** GetIBApplicationDetailsByStatusID End ***************");
