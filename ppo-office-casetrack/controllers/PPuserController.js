...
class PPuserController {
  ...
  static async createPPUser(req, res) {
    ...
    try {
      ...
      const hashedPassword = await bcrypt.hash(UserPassword, 10); // 10 salt rounds
      ...
      await new Promise((resolve, reject) => {
        db.query(query, params, (err, results) => {
          if (err) {
            return ResponseHelper.error(res, "An error occurred while creating PPUser");
          }
          resolve(results);
        });
      });

      // Fetch the output parameters from the stored procedure
